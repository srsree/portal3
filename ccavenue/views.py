...
# amount options
converted_amounts = {'int-1':get_rupee(12), 'int-2':get_rupee(50),'int-3':get_rupee(50),'ind-1':'700', 'ind-2':'3000', 'ind-3':'1800', 'ind-4':'2800','ind-5':'1200','ind-6':'2000','ind-7':'5000','ind-8':'10000','ind-9':'20000','ind-10':'25000','ind-11':'100000'}
...
def bookshopsubscribe(request):
    return render(request,'bookshop_subscribe.html',locals())

def get_display_name(str_obj):
    if str_obj =='ind-2' or str_obj == 'int-3':
        val = 'One Year Immersive School Program'
...
def subscribe1(request, key=''):
    ...
    # your access_code 
    access_code = ccaccess_code # for local
    # for live code
    # access_code = "AVZC00BA44CA22CZAC"
    # for staging code
    # access_code = "AVIV90HA09CG83VIGC"
    # your working_key
    working_key = ccworking_key # local
    # for live key
    # working_key = "2AF08497DFEDB4424306C27C0BFE5102"
    # for staging key 
    # working_key = "2E635E27CE8F901797530D419AD2A7A6"
    ...
    # redirect_url = "http://civilsocietystaging.mahiti.org/subscribe/redirect/"
    redirect_url = ccredirect_url
    ...
    # cancel_url = "http://civilsocietystaging.mahiti.org/subscribe/redirect/"
    cancel_url = cccancel_url
    if request.method == "POST":
        ...
        if cnt:
            ...
            if key:
                BookSubscribers.objects.create(subscription=subobj, amount=p_amount, name=key,language=p_language)
...
def subscribe_redirect(request):
    ...
    # working_key = "2AF08497DFEDB4424306C27C0BFE5102" # this is for live
    # working_key = "2E635E27CE8F901797530D419AD2A7A6" # this is for staging
    working_key =ccworking_key 
 # this for local
    # sub_read(request.POST)
    ...
    if sub:
        ...
        if resdict['order_status'] == "Success":
            if not bs_obj:
                ...
                template = get_template('watcher_subscriber_email_hindi.html')
                subject = "Thank you for buying The Water Catchers."
                context = Context({'username':""})
                content = template.render(context)
                sender_mail = "subscriptions@civilsocietyonline.com"
...
# def sub_read(val):
#     f=open("/home/mahiti/Aswini/Aswini/gitproject/PortalStaging/static/sub.txt", "wa+")
#     f.write("val")
#     f.write(str(val))
#     f.close()
...
def sub_read(val):
    f=open("/home/mahiti/PortalStaging/static/txtfiles/"+str(int(time.time()))+".txt", "w+")
    f.write("val")
    f.write(str(val))
    f.close()
        ...
        if cnt:
            if not key:
                ...
                if cnt.name == 'India':
                    ...
                    shiping_charge = int(profession) * 175
                p_amount = str((280 * int(profession)) + (shiping_charge))
            p_billing_country = cnt.name
            p_billing_tel = request.POST.get('billing_tel')
            p_billing_email = request.POST.get('billing_email')
            subobj = Subscription.objects.create(name = p_billing_name, amount=p_amount, year = year, orderid=p_order_id, status="Pending", 
            ...
            encryption = encrypt(merchant_data,working_key)
        return render(request, 'payment.html', locals())
    return render(request, 'subscribehindi.html', locals())
